...
public class JobRolesDao {
    ...
    public void updateJobRole(int id, JobRoleRequest jobRole, Connection conn) throws SQLException {
        String statement = "UPDATE job_role SET"
                + " job_role_name = ?, job_family_id = ?, management_level_id = ?,"
                + " specification_summary = ?, sharepoint_link = ?, responsibilities = ?"
                + " WHERE job_role_id = ?";
        DaoUtil.executeStatement(conn, statement, false,
                jobRole.getJobRoleName(),
                jobRole.getJobFamilyGroupId(),
                jobRole.getJobBandLevelId(),
                jobRole.getJobRoleLink(),
                jobRole.getJobRoleSpec(),
                jobRole.getJobResponsibilities(),
                id);
    }
}
